...
- Done (slice 24): reequilibrage du panneau filtres `/plans-recipes` en separant `Progression + Type` du bloc `Categories + Sous-categories`, puis ajout d un niveau de detail progression reserve aux sections `fandom` stables de `workshop` et `recipe` (`Beds`, `Water`, `Chems`, `Brewing`, etc.) pour savoir plus vite ce qu il manque sans surcharger encore le catalogue.
