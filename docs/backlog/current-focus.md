...
- Done (slice 42): polish visuel de l ecran admin `catalog/items` pour harmoniser les sous-sections de detail (titres, tableaux, `pre/code`, liens externes et bloc `Canonical signals`) avec le reste du back-office, y compris en theme terminal.
- Done (slice 43): reequilibrage fin des largeurs de colonnes et du wrapping sur `admin/catalog/items` (liste, sources externes, merge et signaux canoniques), afin que les tableaux restent lisibles sur du contenu reel sans surdimensionner les colonnes secondaires.
