...
- Done (slice 36): ajout du champ canonique derive `raid` pour `fallout_wiki` via un matching par token normalise (`Raid: ...`, `... (Raid)`), afin de couvrir les labels generiques de raid sans enumerer chaque activite individuellement.
- Done (slice 37): ajout du champ canonique derive `expeditions` pour `fallout_wiki` via le token generique `Expeditions`, afin de capter les labels de categorie (`Expeditions: The Pitt`, `Expeditions Giuseppe`) sans classer automatiquement toutes les expeditions nommees.
- Done (slice 38): l extraction `fallout_wiki` privilegie maintenant les `icons` quand un payload `{text, icons}` est present, ce qui supprime le bruit concatene des champs `text` et permet de deriver proprement les vendeurs/currencies nommes (`Giuseppe`, `Reginald Stone`, `Regs`, `Minerva`) ainsi que les expeditions evidentes (`Tax Evasion`, `The Human Condition`, `The Most Sensational Game`, `Atlantic City`, `The Pitt`).
- Done (slice 39): le rapport de vocabulaire decoupe maintenant certains `obtained.text` concaténes quand aucun `icons` n est disponible, via un glossaire borne (`Containers`, `World spawns`, `Daily Ops`, `Giuseppe`, etc.), et la normalisation derive aussi `expeditions` pour `Union Dues` ainsi que `purchase_currency=gold_bullion` pour `Samuel (Wastelanders)` et `Bullion vendors`.
- Done (slice 40): ajout du champ canonique derive `random_encounters` pour `fallout_wiki` via les labels explicites `Fallout 76 random encounters` / `Random Encounters`, avec propagation dans le merge cross-source et le rapport de vocabulaire.
- Done (slice 41): l interface admin `/admin/catalog/items` affiche maintenant un bloc `Canonical signals` base sur les decisions du merge, afin de rendre visibles les champs derives retenus (`purchase_currency`, `events`, `expeditions`, `raid`, etc.) sans lire les blobs metadata JSON.
- Done (slice 42): harmonisation visuelle de la page admin `/admin/catalog/items` avec le reste du back-office (titres de section, tableaux de detail, blocs `pre/code`, liens externes, variante terminal), afin que les nouveaux signaux canoniques soient lisibles sans paraitre “hors systeme”.
- Done (slice 43): ajustement fin des proportions de colonnes et du wrapping sur la liste et les tableaux de detail de `/admin/catalog/items`, pour mieux allouer l espace aux URLs, raisons de merge et valeurs JSON sans laisser des colonnes secondaires prendre trop de place.
- Done (slice 44): reequilibrage complementaire des tableaux de `/admin/catalog/items` pour favoriser les colonnes porteuses d information metier (nom d item, URL externe, valeurs retenues) et ajout d une grille dediee au tableau des conflits de merge.
- Done (slice 45): correction de la cascade CSS des tableaux admin catalogue pour surcharger la grille globale `translations-table` et rendre effectives les largeurs dediees par tableau (liste, sources externes, merge, conflits, signaux canoniques).
- Done (slice 46): affinage final des proportions sur la liste et `Sources externes`, afin de donner un maximum de place au nom d item et aux metadata, avec un statut de merge et une URL moins dominants visuellement.
- Done (slice 47): reduction additionnelle de la colonne `merge status` sur la liste `/admin/catalog/items`, pour faire clairement dominer le nom d item sur chaque ligne.
- Done (slice 48): reconciliation `BOOK` par `form_id` lors de l import, avec preference deterministe pour l item deja ancre sur les sources catalogue (`fandom`/`fallout_wiki`), fusion des doublons Minerva existants sur ce keeper (sources externes, listes BOOK, flags locaux) et migration de `player_item_knowledge` avant suppression du doublon, afin d eviter des cas comme `Plan: Cattle prod` presents deux fois en base.
- Done (slice 49): remap et nettoyage des cles de traduction `item.book.<ancienSourceId>.*` devenues orphelines apres la reconciliation par `form_id`, en preservant les meilleures valeurs locales (par exemple DE) sur la cle gardee au lieu de laisser un fallback anglais ou des entrees mortes dans `translations/items.*.yaml`.
